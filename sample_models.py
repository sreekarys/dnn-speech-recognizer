...
def deep_rnn_model(input_dim, units, recur_layers, output_dim=29):
    ...
    rnn_bn_data = input_data
    for i in range(recur_layers):
        rnn = GRU(units, activation='relu', name='rnn_' + str(i+1),
                  return_sequences=True, implementation=2)(rnn_bn_data)
        bn_rnn = BatchNormalization(name='bn_rnn' + str(i+1))(rnn)
        rnn_bn_data = bn_rnn
    # TODO: Add a TimeDistributed(Dense(output_dim)) layer
    time_dense = TimeDistributed(Dense(output_dim))(rnn_bn_data)
    # Add softmax activation layer
    y_pred = Activation('softmax', name='softmax')(time_dense)
    # Specify the model
    model = Model(inputs=input_data, outputs=y_pred)
...
def bidirectional_rnn_model(input_dim, units, output_dim=29):
    ...
    bidir_rnn = Bidirectional(GRU(units, activation='relu',name='bidir_rnn',
